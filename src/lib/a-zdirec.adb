...
with AdaForge.Wide_Wide_Directories.Validity;
with Interfaces.C;
with System;
with System.CRTL;
with System.File_Attributes;
with System.File_IO;
...
use AdaForge.Wide_Wide_Directories.Hierarchical_File_Names;
use AdaForge.Wide_Wide_Directories.Validity;
use System;
use System.CRTL;
use System.File_Attributes;
use System.File_IO;
use System.OS_Constants;
-- use System.OS_Lib;
use System.Regexp;

package body AdaForge.Wide_Wide_Directories is
   ...
   Dir_Separator_C : constant Character;
   pragma Import (C, Dir_Separator_C, "__gnat_dir_separator");
   --  Running system default directory separator
   Dir_Separator : constant Wide_Wide_Character := To_Wide_Wide_Character (Dir_Separator_C);
   Directory_Separator : constant Wide_Wide_Character := To_Wide_Wide_Character (System.OS_Lib.Directory_Separator);

   Dir_Seps : constant Wide_Wide_Character_Set := Strings.Wide_Wide_Maps.To_Set ("/\");
   ...
   function File_Exists (Name : Wide_Wide_String) return Boolean;
   ...
   function Base_Name (Name : Wide_Wide_String) return Wide_Wide_String is
      Simple : constant Wide_Wide_String := Simple_Name (Name);
   ...
   function Compose
     (Containing_Directory : Wide_Wide_String := "";
      ...
      Extension            : Wide_Wide_String := "") return Wide_Wide_String
   is
      Result : Wide_Wide_String (1 .. Containing_Directory'Length +
   ...
   begin
      ...
      then
         raise Name_Error with
           "invalid directory path name """ & Encode (Containing_Directory, UTF_8) & '"';

      elsif Extension'Length = 0 and then not Is_Valid_Simple_Name (Name) then
         raise Name_Error with
           "invalid simple name """ & Encode (Name, UTF_8) & '"';

      elsif Extension'Length /= 0
        and then not Is_Valid_Simple_Name (Name & '.' & Extension)
      then
         raise Name_Error with
           "invalid file name """ & Encode (Name, UTF_8) & '.' & Encode (Extension, UTF_8) & '"';
...
function Containing_Directory (Name : Wide_Wide_String) return Wide_Wide_String is
   begin
      --  First, the invalid case

      if not Is_Valid_Path_Name (Name) then
         raise Name_Error with "invalid path name """ & Encode (Name, UTF_8) & '"';

      else
         declare
            Last_DS : constant Natural :=
               Index (Source => Name, Set => Dir_Seps, Going => Strings.Backward);
         ...
         begin
            ...
            then
               raise Use_Error with
                 "directory """ & Encode (Name, UTF_8) & """ has no containing directory";
               ...
               declare
                  ...
                  Result : Wide_Wide_String (1 .. Last);
   ...
   is
      Success  : Boolean;
      Mode     : System.OS_Lib.Copy_Mode := System.OS_Lib.Overwrite;
      Preserve : System.OS_Lib.Attribute := System.OS_Lib.None;
   ...
   begin
      ...
      if not Is_Valid_Path_Name (Source_Name) then
         raise Name_Error with
           "invalid source path name """ & Encode (Source_Name, UTF_8) & '"';

      elsif not Is_Valid_Path_Name (Target_Name) then
         raise Name_Error with
           "invalid target path name """ & Encode (Target_Name, UTF_8) & '"';

      elsif not System.OS_Lib.Is_Regular_File (Encode (Source_Name, UTF_8)) then
         raise Name_Error with '"' & Encode (Source_Name, UTF_8) & """ is not a file";

      elsif System.OS_Lib.Is_Directory (Encode (Target_Name, UTF_8)) then
         raise Use_Error with "target """ & Encode (Target_Name, UTF_8) & """ is a directory";
         ...
         if Form'Length > 0 then
            ...
            begin
               ...
               --  Convert form Wide_Wide_String to lower case
               ...
               if V1 = 0 then
                  Preserve := System.OS_Lib.None;
               elsif Formstr (V1 .. V2) = "timestamps" then
                  Preserve := System.OS_Lib.Time_Stamps;
               elsif Formstr (V1 .. V2) = "all_attributes" then
                  Preserve := System.OS_Lib.Full;
         ...
         System.OS_Lib.Copy_File (Encode (Source_Name, UTF_8), Encode (Target_Name, UTF_8), Success, Mode, Preserve);

         if not Success then
            raise Use_Error with "copy of """ & Encode (Source_Name, UTF_8) & """ failed";
         end if;
      end if;
   end Copy_File;
   ...
   procedure Create_Directory
     (New_Directory : Wide_Wide_String;
      Form          : String := "utf8")
   is
      -- Dir_Name_C : constant UTF_8_String := Encode (New_Directory, UTF_8) & ASCII.NUL;
      Dir_Name_C : constant String := To_String (New_Directory) & ASCII.NUL;
   ...
   begin
      ...
      if not Is_Valid_Path_Name (New_Directory) then
         raise Name_Error with
--           "invalid new directory path name """ & Encode (New_Directory, UTF_8) & '"';
           "invalid new directory path name """ & To_String (New_Directory) & '"';
         ...
         begin
            ...
            if CRTL.mkdir (Dir_Name_C, Encoding) /= 0 then
               raise Use_Error with
                 "creation of new directory """ & To_String (New_Directory) & """ failed";
            end if;
         end;
      end if;
   ...
   procedure Create_Path
     (New_Directory : Wide_Wide_String;
      Form          : String := "")
   is
      New_Dir : Wide_Wide_String (1 .. New_Directory'Length + 1);
      Last    : Positive := 1;
      Start   : Positive := 1;
      ...
      if not Is_Valid_Path_Name (New_Directory) then
         ...
         for J in Start + 1 .. New_Dir'Last loop
            ...
            --  a first directory separator.
               ...
               if not System.OS_Lib.Is_Directory (Encode (New_Dir (1 .. Last), UTF_8)) then
                  begin
                     Create_Directory
                  ...
                  exception
                     when Use_Error =>
                        if File_Exists (New_Dir (1 .. Last)) then
                           ...
                           if not System.OS_Lib.Is_Directory (Encode (New_Dir (1 .. Last), UTF_8)) then
                              raise Use_Error with
                                "file """ & Encode (New_Dir (1 .. Last), UTF_8) &
   ...
   function Current_Directory return Wide_Wide_String is
   ...
   begin
      ...
      return Decode (System.OS_Lib.Normalize_Pathname (Buffer (1 .. Path_Len)), UTF_8);
   ...
   procedure Delete_Directory (Directory : Wide_Wide_String) is
   begin
      --  First, the invalid cases

      if not Is_Valid_Path_Name (Directory) then
         raise Name_Error with
           "invalid directory path name """ & Encode (Directory, UTF_8) & '"';

      elsif not System.OS_Lib.Is_Directory (Encode (Directory, UTF_8)) then
         raise Name_Error with '"' & Encode (Directory, UTF_8) & """ not a directory";

      --  Do the deletion, checking for error

      else
         declare
            Dir_Name_C : constant UTF_8_String := Encode (Directory, UTF_8) & ASCII.NUL;
         begin
            if rmdir (Dir_Name_C) /= 0 then
               raise Use_Error with
                 "deletion of directory """ & Encode (Directory, UTF_8) & """ failed";
            end if;
         end;
      end if;
   ...
   procedure Delete_File (Name : Wide_Wide_String) is
      Success : Boolean;
   ...
   begin
      ...
      elsif not System.OS_Lib.Is_Regular_File (Encode (Name, UTF_8))
        and then not System.OS_Lib.Is_Symbolic_Link (Encode (Name, UTF_8))
      then
         raise Name_Error with "file """ & Encode (Name, UTF_8) & """ does not exist";

      else
         --  Do actual deletion using System.OS_Lib.Delete_File

         System.OS_Lib.Delete_File (Encode (Name, UTF_8), Success);

         if not Success then
            raise Use_Error with "file """ & Encode (Name, UTF_8) & """ could not be deleted";
         end if;
      end if;
   end Delete_File;
   ...
   procedure Delete_Tree (Directory : Wide_Wide_String) is
      Search      : Search_Type;
   ...
   begin
      ...
      if not Is_Valid_Path_Name (Directory) then
         ...
         while More_Entries (Search) loop
            ...
            declare
               Fname : constant Wide_Wide_String := Full_Name   (Dir_Ent);
               Sname : constant Wide_Wide_String := Simple_Name (Dir_Ent);

            begin
               if OS_Lib.Is_Directory (Encode (Fname, UTF_8)) then
            ...
            if rmdir (Dir_Name_C) /= 0 then
               raise Use_Error with
                 "directory tree rooted at """ &
                   Encode (Directory, UTF_8) & """ could not be deleted";
            end if;
         end;
      end if;
   end Delete_Tree;
   ...
   function Extension (Name : Wide_Wide_String) return Wide_Wide_String is
   begin
      --  First, the invalid case
      ...
      if not Is_Valid_Path_Name (Name) then
         ...
         for Pos in reverse Name'Range loop
            ...
            if Is_In (Name (Pos), Dir_Seps) then
               ...
               declare
                  subtype Result_Type is Wide_Wide_String (1 .. Name'Last - Pos);
               begin
   ...
   function File_Exists (Name : Wide_Wide_String) return Boolean is
   ...
   begin
      C_Name (1 .. Name'Length) := Encode (Name, UTF_8);
   ...
   function Full_Name (Name : Wide_Wide_String) return Wide_Wide_String is
   begin
      --  First, the invalid case
      ...
      if not Is_Valid_Path_Name (Name) then
         ...
         declare
            ...
            Value : constant Wide_Wide_String := Decode (System.OS_Lib.Normalize_Pathname (Encode (Name, UTF_8)), UTF_8);
            subtype Result is Wide_Wide_String (1 .. Value'Length);
   ...
   function Full_Name (Directory_Entry : Directory_Entry_Type) return Wide_Wide_String is
   begin
      ...
      if not Directory_Entry.Valid then
         ...
         return To_Wide_Wide_String (Directory_Entry.Full_Name);
      end if;
   end Full_Name;
   ...
   begin
      ...
      if Directory_Entry.Attr_Error_Code /= 0 then
         raise Use_Error
           with Encode (To_Wide_Wide_String (Directory_Entry.Full_Name), UTF_8) & ": " &
             System.OS_Lib.Errno_Message (Err => Directory_Entry.Attr_Error_Code);
      end if;
   end Get_Next_Entry;
   ...
   function Kind (Name : Wide_Wide_String) return File_Kind is
   begin
      --  First, the invalid case
      ...
      elsif System.OS_Lib.Is_Regular_File (Encode (Name, UTF_8)) then
         return Ordinary_File;

      elsif System.OS_Lib.Is_Directory (Encode (Name, UTF_8)) then
         return Directory;
   ...
   function Modification_Time (Name : Wide_Wide_String) return Time is
   ...
   begin
      ...
      if not (System.OS_Lib.Is_Regular_File (Encode (Name, UTF_8)) or else System.OS_Lib.Is_Directory (Encode (Name, UTF_8))) then
         raise Name_Error with '"' & Encode (Name, UTF_8) & """ not a file or directory";

      else
         C_Name := Encode (Name, UTF_8) & ASCII.NUL;
         Date := C_Modification_Time (C_Name'Address);

         if Date = Invalid_Time then
            raise Use_Error with
              "Unable to get modification time of the file """ & Encode (Name, UTF_8) & '"';
         end if;
   ...
   function Name_Case_Equivalence (Name : Wide_Wide_String) return Name_Case_Kind is
      Dir_Path  : Unbounded_Wide_Wide_StrIng := To_Unbounded_Wide_Wide_StrIng (Name);
      S         : Search_Type;
   ...
   begin
      ...
      if System.OS_Lib.Is_Regular_File (Encode (Name, UTF_8)) then
         Dir_Path := To_Unbounded_Wide_Wide_StrIng (Containing_Directory (Name));
      end if;
      ...
      Start_Search
        (Search    => S,
         Directory => To_Wide_Wide_String (Dir_Path),
         Pattern   => "",
      ...
      if Exists (To_Wide_Wide_String (Dir_Path) & Directory_Separator
                  & To_Lower (Simple_Name (Test_File)))
        and then Exists (To_Wide_Wide_String (Dir_Path) & Directory_Separator
                          & To_Upper (Simple_Name (Test_File)))
      then
   ...
   procedure Rename (Old_Name, New_Name : Wide_Wide_String) is
      Success : Boolean;
   ...
   begin
      ...
      if not Is_Valid_Path_Name (Old_Name) then
         raise Name_Error with "invalid old path name """ & Encode (Old_Name, UTF_8) & '"';

      elsif not Is_Valid_Path_Name (New_Name) then
         raise Name_Error with "invalid new path name """ & Encode (New_Name, UTF_8) & '"';

      elsif not System.OS_Lib.Is_Regular_File (Encode (Old_Name, UTF_8))
            and then not System.OS_Lib.Is_Directory (Encode (Old_Name, UTF_8))
      then
         raise Name_Error with "old file """ & Encode (Old_Name, UTF_8) & """ does not exist";

      elsif System.OS_Lib.Is_Regular_File (Encode (New_Name, UTF_8)) or else System.OS_Lib.Is_Directory (Encode (New_Name, UTF_8)) then
         raise Use_Error with
           "new name """ & Encode (New_Name, UTF_8)
           & """ designates a file that already exists";

      --  Do actual rename using System.OS_Lib.Rename_File

      else
         System.OS_Lib.Rename_File (Encode (Old_Name, UTF_8), Encode (New_Name, UTF_8), Success);
         ...
         if not Success then
            ...
            if System.OS_Lib.Errno = ENOENT then
               raise Name_Error with
                 "file """ & Encode (Containing_Directory (New_Name), UTF_8) & """ not found";

            else
               raise Use_Error with
                 "file """ & Encode (Old_Name, UTF_8) & """ could not be renamed";
            end if;
         end if;
      end if;
   end Rename;
   ...
   procedure Set_Directory (Directory : Wide_Wide_String) is
      Dir_Name_C : constant UTF_8_String := Encode (Directory, UTF_8) & ASCII.NUL;
   begin
      if not Is_Valid_Path_Name (Directory) then
         raise Name_Error with
         ...
         raise Name_Error with
           "directory """ & Encode (Directory, UTF_8) & """ does not exist";

      elsif chdir (Dir_Name_C) /= 0 then
         raise Name_Error with
           "could not set to designated directory """ & Encode (Directory, UTF_8) & '"';
      end if;
   end Set_Directory;
   ...
   function Simple_Name (Name : Wide_Wide_String) return Wide_Wide_String is

      function Simple_Name_Internal (Path : Wide_Wide_String) return Wide_Wide_String;
      ...
      function Simple_Name_Internal (Path : Wide_Wide_String) return Wide_Wide_String is
         Cut_Start : Natural :=
           Index (Source => Path, Set => Dir_Seps, Going => Strings.Backward);
      ...
      begin
         ...
         if Cut_Start = Path'Last then
            Cut_End   := Path'Last - 1;
            Cut_Start := Index
                           (Source => Path (Path'First .. Path'Last - 1),
                           Set => Dir_Seps,
         ...
         Check_For_Standard_Dirs : declare
            BN : constant Wide_Wide_String := Path (Cut_Start .. Cut_End);
         ...
         begin
            if BN = "." or else BN = ".." then
              ...
              and then Wide_Wide_Characters.Handling.Is_Letter (BN (BN'First))
   ...
   begin
      ...
      if not Is_Valid_Path_Name (Name) then
         ...
         declare
            Value : constant Wide_Wide_String := Simple_Name_Internal (Name);
            subtype Result is Wide_Wide_String (1 .. Value'Length);
         begin
            return Result (Value);
         end;
   ...
   function Simple_Name
     (Directory_Entry : Directory_Entry_Type) return Wide_Wide_String is
   begin
      ...
      if not Directory_Entry.Valid then
         ...
         return To_Wide_Wide_String (Directory_Entry.Name);
      end if;
   end Simple_Name;
   ...
   function Size (Name : Wide_Wide_String) return File_Size is
      C_Name : String (1 .. Name'Length + 1);
   ...
   begin
      ...
      if not System.OS_Lib.Is_Regular_File (Encode (Name, UTF_8)) then
         raise Name_Error with "file """ & Encode (Name, UTF_8) & """ does not exist";
   ...
   begin
      ...
      if not System.OS_Lib.Is_Directory (Encode (Directory, UTF_8)) then
         raise Name_Error with
           "unknown directory """ &  Encode (Simple_Name (Directory), UTF_8) & '"';
      end if;
      ...
      begin
         ...
         Pattern_Regex :=
           Compile (Encode (Pattern, UTF_8), Glob => True, Case_Sensitive => Case_Sensitive);
      exception
         when Error_In_Regexp =>
            raise Name_Error with "invalid pattern """ & Encode (Pattern, UTF_8) & '"';
      end;
      ...
      if Dir_Pointer = No_Dir then
         raise Use_Error with
           "unreadable directory """ & Encode (Simple_Name (Directory), UTF_8) & '"';
      end if;
      ...
      loop
         ...
         declare
            File_Name_C : constant String (1 .. File_Name_Len)
              with Import, Address => File_Name_Addr;
            File_Name : constant Wide_Wide_String := Decode (File_Name_C, UTF_8);
         begin
            if Match (Encode (File_Name, UTF_8), Pattern_Regex) then
               declare
                  Path_C : constant UTF_8_String :=
                             Encode (Compose (Directory, File_Name), UTF_8) & ASCII.NUL;
                  Path   : Wide_Wide_String -- renames
                             := To_Wide_Wide_String (Path_C (Path_C'First .. Path_C'Last - 1));
                  ...
                  if Error /= 0 then
                     Search.State.Dir_Contents.Append
                       (Directory_Entry_Type'
                          (Valid           => True,
                           Name            => To_Unbounded_Wide_Wide_StrIng (File_Name),
                           Full_Name       => To_Unbounded_Wide_Wide_StrIng (Path),
                     ...
                     if Res.Found then
                        Search.State.Dir_Contents.Append
                          (Directory_Entry_Type'
                             (Valid             => True,
                              Name              =>
                                To_Unbounded_Wide_Wide_StrIng (File_Name),
                              Full_Name         => To_Unbounded_Wide_Wide_StrIng (Path),
                              Attr_Error_Code   => 0,
                              Kind              => Res.Kind,
                              Modification_Time =>
...
end AdaForge.Wide_Wide_Directories;
