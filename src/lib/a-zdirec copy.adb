   ...
   begin
      ...
      then
         raise Name_Error with
           "invalid directory path name """ & To_String (Containing_Directory) & '"';

      elsif Extension'Length = 0 and then not Is_Valid_Simple_Name (Name) then
         raise Name_Error with
           "invalid simple name """ & To_String (Name) & '"';

      elsif Extension'Length /= 0
        and then not Is_Valid_Simple_Name (Name & '.' & Extension)
      then
         raise Name_Error with
           "invalid file name """ & To_String (Name) & '.' & To_String (Extension) & '"';
...
function Containing_Directory (Name : Wide_Wide_String) return Wide_Wide_String is
   begin
      ...
      if not Is_Valid_Path_Name (Name) then
         raise Name_Error with "invalid path name """ & To_String (Name) & '"';
         ...
         begin
            ...
            then
               raise Use_Error with
                 "directory """ & To_String (Name) & """ has no containing directory";
   ...
   begin
      ...
      if not Is_Valid_Path_Name (Source_Name) then
         raise Name_Error with
           "invalid source path name """ & To_String (Source_Name) & '"';

      elsif not Is_Valid_Path_Name (Target_Name) then
         raise Name_Error with
           "invalid target path name """ & To_String (Target_Name) & '"';

      elsif not System.OS_Lib.Is_Regular_File (To_String (Source_Name)) then
         raise Name_Error with '"' & To_String (Source_Name) & """ is not a file";

      elsif System.OS_Lib.Is_Directory (To_String (Target_Name)) then
         raise Use_Error with "target """ & To_String (Target_Name) & """ is a directory";
         ...
         System.OS_Lib.Copy_File (To_String (Source_Name), To_String (Target_Name), Success, Mode, Preserve);

         if not Success then
            raise Use_Error with "copy of """ & To_String (Source_Name) & """ failed";
         end if;
      end if;
   end Copy_File;
   ...
   procedure Create_Directory
     (New_Directory : Wide_Wide_String;
      Form          : String := "utf8")
   is
      -- Dir_Name_C : constant UTF_8_String := To_String (New_Directory) & ASCII.NUL;
      ...
      if not Is_Valid_Path_Name (New_Directory) then
         ...
         for J in Start + 1 .. New_Dir'Last loop
            ...
            --  a first directory separator.
               ...
               if not System.OS_Lib.Is_Directory (To_String (New_Dir (1 .. Last))) then
                  begin
                     Create_Directory
   ...
   begin
      ...
      return Decode (System.OS_Lib.Normalize_Pathname (Buffer (1 .. Path_Len)));
   ...
   begin
      ...
      if not Is_Valid_Path_Name (Directory) then
         raise Name_Error with
           "invalid directory path name """ & To_String (Directory) & '"';

      elsif not System.OS_Lib.Is_Directory (To_String (Directory)) then
         raise Name_Error with '"' & To_String (Directory) & """ not a directory";

      --  Do the deletion, checking for error

      else
         declare
            Dir_Name_C : constant UTF_8_String := To_String (Directory) & ASCII.NUL;
         begin
            if rmdir (Dir_Name_C) /= 0 then
               raise Use_Error with
                 "deletion of directory """ & To_String (Directory) & """ failed";
            end if;
         end;
      end if;
   ...
   begin
      ...
      elsif not System.OS_Lib.Is_Regular_File (To_String (Name))
        and then not System.OS_Lib.Is_Symbolic_Link (To_String (Name))
      then
         raise Name_Error with "file """ & To_String (Name) & """ does not exist";

      else
         --  Do actual deletion using System.OS_Lib.Delete_File

         System.OS_Lib.Delete_File (To_String (Name), Success);

         if not Success then
            raise Use_Error with "file """ & To_String (Name) & """ could not be deleted";
         end if;
      end if;
   end Delete_File;
   ...
   begin
      ...
      if not Is_Valid_Path_Name (Directory) then
         ...
         while More_Entries (Search) loop
            ...
            begin
               if OS_Lib.Is_Directory (To_String (Fname)) then
            ...
            if rmdir (Dir_Name_C) /= 0 then
               raise Use_Error with
                 "directory tree rooted at """ &
                   To_String (Directory) & """ could not be deleted";
            end if;
         end;
      end if;
   end Delete_Tree;
   ...
   begin
      C_Name (1 .. Name'Length) := To_String (Name);
   ...
   begin
      ...
      if not Is_Valid_Path_Name (Name) then
         ...
         declare
            ...
            Value : constant Wide_Wide_String := Decode (System.OS_Lib.Normalize_Pathname (To_String (Name)));
   ...
   begin
      ...
      if Directory_Entry.Attr_Error_Code /= 0 then
         raise Use_Error
           with To_String (To_Wide_Wide_String (Directory_Entry.Full_Name)) & ": " &
   ...
   begin
      ...
      elsif System.OS_Lib.Is_Regular_File (To_String (Name)) then
         return Ordinary_File;

      elsif System.OS_Lib.Is_Directory (To_String (Name)) then
         return Directory;
   ...
   begin
      ...
      if not (System.OS_Lib.Is_Regular_File (To_String (Name)) or else System.OS_Lib.Is_Directory (To_String (Name))) then
         raise Name_Error with '"' & To_String (Name) & """ not a file or directory";

      else
         C_Name := To_String (Name) & ASCII.NUL;
         Date := C_Modification_Time (C_Name'Address);

         if Date = Invalid_Time then
            raise Use_Error with
              "Unable to get modification time of the file """ & To_String (Name) & '"';
         end if;
   ...
   begin
      ...
      if System.OS_Lib.Is_Regular_File (To_String (Name)) then
   ...
   begin
      ...
      if not Is_Valid_Path_Name (Old_Name) then
         raise Name_Error with "invalid old path name """ & To_String (Old_Name) & '"';

      elsif not Is_Valid_Path_Name (New_Name) then
         raise Name_Error with "invalid new path name """ & To_String (New_Name) & '"';

      elsif not System.OS_Lib.Is_Regular_File (To_String (Old_Name))
            and then not System.OS_Lib.Is_Directory (To_String (Old_Name))
      then
         raise Name_Error with "old file """ & To_String (Old_Name) & """ does not exist";

      elsif System.OS_Lib.Is_Regular_File (To_String (New_Name)) or else System.OS_Lib.Is_Directory (To_String (New_Name)) then
         raise Use_Error with
           "new name """ & To_String (New_Name)
           & """ designates a file that already exists";

      --  Do actual rename using System.OS_Lib.Rename_File

      else
         System.OS_Lib.Rename_File (To_String (Old_Name), To_String (New_Name), Success);
         ...
         if not Success then
            ...
            if System.OS_Lib.Errno = ENOENT then
               raise Name_Error with
                 "file """ & To_String (Containing_Directory (New_Name)) & """ not found";

            else
               raise Use_Error with
                 "file """ & To_String (Old_Name) & """ could not be renamed";
            end if;
         end if;
      end if;
   end Rename;
   ...
   begin
      if not Is_Valid_Path_Name (Directory) then
         ...
         raise Name_Error with
           "directory """ & To_String (Directory) & """ does not exist";

      elsif chdir (Dir_Name_C) /= 0 then
         raise Name_Error with
           "could not set to designated directory """ & To_String (Directory) & '"';
      end if;
   end Set_Directory;
   ...
   begin
      ...
      if not System.OS_Lib.Is_Regular_File (To_String (Name)) then
         raise Name_Error with "file """ & To_String (Name) & """ does not exist";
   ...
   begin
      ...
      if not System.OS_Lib.Is_Directory (To_String (Directory)) then
         raise Name_Error with
           "unknown directory """ &  To_String (Simple_Name (Directory)) & '"';
      end if;
      ...
      begin
         ...
         Pattern_Regex :=
           Compile (To_String (Pattern), Glob => True, Case_Sensitive => Case_Sensitive);
      exception
         when Error_In_Regexp =>
            raise Name_Error with "invalid pattern """ & To_String (Pattern) & '"';
      end;
      ...
      if Dir_Pointer = No_Dir then
         raise Use_Error with
           "unreadable directory """ & To_String (Simple_Name (Directory)) & '"';
      end if;
      ...
      loop
         ...
         declare
            ...
            File_Name : constant Wide_Wide_String := Decode (File_Name_C);
         begin
            if Match (To_String (File_Name), Pattern_Regex) then
               declare
                  Path_C : constant UTF_8_String :=
                             To_String (Compose (Directory, File_Name)) & ASCII.NUL;
