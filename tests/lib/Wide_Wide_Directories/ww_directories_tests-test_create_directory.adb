...
procedure Test_Create_Directory (T : in out UnitTest.Test_Case'Class) is --UnitTest.Test_Case'Class) is
...
begin
   ...
   for Some_String_Kind in String_Kind'Range loop
      ...
      begin
         ...
         Ada.Wide_Wide_Text_IO.Put_Line ("Loop create (" & Some_String_Kind'Wide_Wide_Image & "): " & Base_Dir_Name);
         AdaForge.Wide_Wide_Directories.Create_Directory (
            New_Directory => Base_Dir_Name,
            Form          => "utf8");

         UnitTest.Assert (True, "Failed Create_Directory (""" 
            & Base_Dir_Name & """)");
      exception
         when others =>
            UnitTest.Fail ("Exception: Failed Create_Directory !! (""" & Base_Dir_Name & """)");
      end;
   end loop;
